...
public class FscBleCentralCallbacksImpInformation extends FscBleCentralCallbacksImp {
    ...
    private static final String TAG = "FscBleCentralCallbacksI";
    @Override
...
//        LogUtil.i("ble", command);
        if (CommandBean.COMMAND_BEGIN.equals(command)) {
            ...
            if (status == CommandBean.COMMAND_SUCCESSFUL) {
                if (command.contains("=")) {
                    /**
                     *  modify parameter 改变NAME
                     */
                    Log.e(TAG, "atCommandCallBack: " +command+ "      "+ command.substring(0,command.indexOf("=")));
...
//        activityWeakReference.get().isDisConnect = false;
        /**
...
//        activityWeakReference.get().isDisConnect = true;
